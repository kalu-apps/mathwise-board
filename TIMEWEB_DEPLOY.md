...
- `mw-media-01` — coturn (`turn.board.mathwise.ru`) и при необходимости LiveKit (`rtc.board.mathwise.ru`);
...
## 5) LiveKit (опционально, для групп >2)
Если нужен SFU:
- поднимается на `mw-media-01`;
- `rtc.board.your-domain.tld` проксируется на порт LiveKit;
- в board backend добавляется выдача LiveKit token/room.

Текущий репозиторий уже поддерживает p2p WebRTC + TURN (без обязательного LiveKit).
